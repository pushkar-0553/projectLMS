...
-- This script migrates existing data to the new platform schema
...
CREATE TABLE IF NOT EXISTS Steps_backup AS SELECT * FROM Steps;
CREATE TABLE IF NOT EXISTS Progress_backup AS SELECT * FROM Progress;
CREATE TABLE IF NOT EXISTS StudentProgress_backup AS SELECT * FROM StudentProgress;
...
-- =============================================
-- STEP 3: CREATE NEW PLATFORM TABLES
-- =============================================
...
-- =============================================
-- STEP 6: UPDATE EXISTING TRIGGERS AND PROCEDURES
-- =============================================

-- Create trigger to update batch student count
DELIMITER //
CREATE TRIGGER IF NOT EXISTS update_batch_student_count
AFTER INSERT ON StudentBatches
FOR EACH ROW
BEGIN
    UPDATE Batches 
    SET current_students = (
        SELECT COUNT(*) FROM StudentBatches 
...
-- Create trigger to update batch student count on status change
DELIMITER //
CREATE TRIGGER IF NOT EXISTS update_batch_student_count_status
AFTER UPDATE ON StudentBatches
FOR EACH ROW
BEGIN
...
UNION ALL
SELECT 
    'Batches' as table_name, COUNT(*) as record_count FROM Batches
UNION ALL
SELECT 
    'StudentBatches' as table_name, COUNT(*) as record_count FROM StudentBatches
UNION ALL
SELECT 
    'LiveSessions' as table_name, COUNT(*) as record_count FROM LiveSessions
UNION ALL
SELECT 
...
SELECT 
    'Orphaned Projects' as issue, COUNT(*) as count 
FROM Projects p 
LEFT JOIN Users u ON p.created_by = u.id 
WHERE u.id IS NULL
UNION ALL
SELECT 
    'Orphaned StudentBatches' as issue, COUNT(*) as count 
FROM StudentBatches sb 
LEFT JOIN Users u ON sb.student_id = u.id 
WHERE u.id IS NULL
UNION ALL
SELECT 
    'Orphaned StudentBatches (Batch)' as issue, COUNT(*) as count 
FROM StudentBatches sb 
